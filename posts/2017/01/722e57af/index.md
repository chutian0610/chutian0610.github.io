...
class Builder
&lt;&lt;interface>> Builder
Builder: +buildPartA() Void
Builder: +buildPartB() Void
Builder: +build() Product
class ConcreteBuilder
ConcreteBuilder: +buildPartA() Void
ConcreteBuilder: +buildPartB() Void
ConcreteBuilder: +build() Product
class Product
Product ..> ConcreteBuilder : Dependency
Builder <|.. ConcreteBuilder : Realization
...
1. 封装性，在建造者模式中，客户端不必知道产品内部组成的细节，将产品本身与产品的创建过程解耦，使得相同的创建过程可以创建不同的产品对象。
