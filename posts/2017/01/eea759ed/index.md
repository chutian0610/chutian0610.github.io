...
Handler:+canConsume(Request request)* Boolean
Handler:+dispatchRequest(Request request) Response
class ConcreteHandlerA
ConcreteHandlerA:+handle(Request request) Response
ConcreteHandlerA:+canConsume(Request request) Boolean
class ConcreteHandlerB
ConcreteHandlerB:+handle(Request request) Response
ConcreteHandlerB:+canConsume(Request request) Boolean
...
abstract class Handler {  
  ...
  // 判断是否可以消费
  public abstract boolean canConsume(Request request);
  // 处理请求的业务代码    
  public abstract Response handle(Request request);  

  // 模板方法
  public final Response dispatchRequest(Request request){
    if(this.canConsume(request)){
      return this.handle(request);
    }else{
      // 注意，链尾handler 能消费所有。
      return this.next.dispatchRequest(request);
    }
  }
}  
